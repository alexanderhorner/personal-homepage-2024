...
function Header() {
  return (
    <header className="flex items-center justify-left gap-4 px-5 py-6 md:px-6 lg:px-8">
      ...
      <a
        href="https://www.linkedin.com/in/alexander-horner-5ba3a31a0"
      ...
      <a
        href="https://github.com/alexanderhorner"
...
function HeroSection() {
  return (
    <section
      className=" bg-cover bg-center lg:text-center relative
...
function PortfolioItemCard({ id, title, img, description }: PortfolioItemProps) {
  return (
    <Link to="/$articleId" params={{ articleId: id }} className="block" resetScroll={false}>
      <div className="bg-white rounded-md overflow-hidden block shadow-xl hover:shadow-2xl hover:scale-[1.01] transition-all duration-500">
        {img ? (
          <div>
            <img loading="lazy" src={img} alt="Project Thumbnail" className="w-full aspect-video object-cover" width={1920} height={1080} />
          </div>
        ) : null}

        <div className="p-4">
          <h3 className="font-bold leading-snug truncate">{title}</h3>
          <div className="text-gray-600 mt-1 line-clamp-2 leading-snug">{description}</div>
        </div>
      </div>
    </Link>
  )
  ...
  return (
    <footer className="bg-gray-50 text-gray-600 py-10 text-center">
      <div className="text-sm">
        ...
        <Link to="/$articleId" params={{ articleId: 'privacy' }} resetScroll={false} className="underline">
          Privacy
        </Link>
      </div>
    </footer>
